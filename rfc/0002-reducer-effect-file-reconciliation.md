...
keyed debounce 以 1 秒 timeout 投递类型化 `ReloadRequested` event。
...
- completion 在同一次 reducer 转换中读取并清除 invalidated，再声明对应的 debounce
  effect。
- file watcher 的正常 inotify 通知使用 1 秒 timeout 和同一个 key，因此会替换 30 秒
...
18. inotify notification 必须以 1 秒 timeout 使用同一个 key，可以替换 30 秒 fallback
    deadline。
...
16. invalidated completion 以 1 秒、未 invalidated completion 以 30 秒续订 reload，两者
    都包括 stat 短路。
17. inotify notification 使用相同 key 和 1 秒 timeout，能够提前替换 30 秒 fallback。
...
inotify 通知使用同一个 key 和 1 秒 timeout，可以替换 30 秒 fallback deadline 并及时
触发。
...
- invalidated completion 的 1 秒、inotify 的 1 秒和 fallback 的 30 秒 timeout 是否需要
  通过 bootstrap 配置？
